...
TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
  ...
  oPrintf("input = %x output = %x filter = %x bias = %x\n", input, output, filter, bias);
  oPrintf("input->data.raw = %x output->data.raw = %x filter->data.raw = %x bias->data.raw = %x\n", input->data.raw, output->data.raw, filter->data.raw, bias->data.raw);
  oPrintf("Eval input->type = %x\n", input->type);
  switch (input->type & 0xFF) {  // Already know in/out types are same.
    ...
    case kTfLiteInt8:
      ...
      break;
    case kTfLiteUInt8:
	  oPrintf("node->custom_initial_data_size=%d\n", node->custom_initial_data_size);
      if (node->custom_initial_data_size == 0) {
        ...
        EvalQuantizedFPGA_SIMD(context, node, params, data, input, filter, bias, nullptr,
		  ...
		  oPrintf("ERROR: Unknown accel type = %x\n", node->custom_initial_data_size);
	  }
      break;
    default:
